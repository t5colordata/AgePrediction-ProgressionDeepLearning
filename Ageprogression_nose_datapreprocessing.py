...
faceCascade=cv2.CascadeClassifier('../nose.xml')
img_train=[]
img_train[:]=[]
processed_train=[]
processed_train[:]=[]
...
for i in range(0,len(X_train1)):
   ...
   faceCascade=cv2.CascadeClassifier('../nose.xml')
   gray=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
   nose = faceCascade.detectMultiScale(gray,scaleFactor=1.4,minNeighbors=3,minSize=(30,30),flags = cv2.cv.CV_HAAR_SCALE_IMAGE)
   try:
      x,y,w,h=nose[0]
      crop_img=gray[y:y+h,x:x+w]
      processed_x_train1_nose.append(X_train1[i])
      pil_image=Image.fromarray(crop_img)
      pl=pil_image.resize(size,Image.ANTIALIAS)
      #pil_image.thumbnail(size,Image.ANTIALIAS)
      data=np.asarray( pl, dtype="int32")
      data.shape
      X_train1_nose.append(data)
   except:
      print ''
...
for i in range(0,len(X_train2)):
   ...
   try:
      x,y,w,h=nose[0]
      crop_img=gray[y:y+h,x:x+w]
      processed_x_train2_nose.append(X_train2[i])
      pil_image=Image.fromarray(crop_img)
      pl=pil_image.resize(size,Image.ANTIALIAS)
      data=np.asarray( pl, dtype="int32")
      data.shape
      X_train2_nose.append(data)
   except Exception as e:
      print e

img_tr1=X_train1_nose[0:12000]
len(img_tr1)
img_ts1=X_train1_nose[12000:14042]
len(img_ts1)
img_tr2=X_train2_nose[0:12000]
len(img_tr2)
img_ts2=X_train2_nose[12000:14042]
len(img_ts2)
...
np.save('fgnet_xtrain1_nose.npy',XTrain1)
np.save('fgnet_xtrain2_nose.npy',XTrain2)
np.save('fgnet_xtest1_nose.npy',XTest1)
np.save('fgnet_xtest2_nose.npy',XTest2)
np.save('fgnet_ytrain_nose.npy',y_train2)
np.save('fgnet_ytest_nose.npy',y_test2)